...
class Validate
{
     ...
     *  @detail: 
     */
    protected function captcha($value, $rule, $data, $field, $title){
        $rid = request()->server('HTTP_REFERER', '');
        $cap = new \think\Captcha;
        $eax = $cap->check($value, $rid);
        return $eax;
    }
    /**
     *  @brief : 验证手机号码
